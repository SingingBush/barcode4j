...
public class DefaultTwoDimCanvasLogicHandler implements TwoDimBarcodeLogicHandler {
     ...
     */
    public DefaultTwoDimCanvasLogicHandler(@NotNull final AbstractBarcodeBean bcBean, @NotNull final Canvas canvas) {
        this.bcBean = bcBean;
        this.canvas = canvas;
    }

    private double getStartX() {
        return bcBean.hasQuietZone() ? bcBean.getQuietZone() : 0.0;
    }

    private double getStartY() {
        return bcBean.hasQuietZone() ? bcBean.getVerticalQuietZone() : 0.0;
    }
