...
//		loadProperties();
	...
	public static synchronized void loadProperties(){
		...
		try {
			InputStream is = EAN128AI.class.getClassLoader().getResourceAsStream("org/krysalis/barcode4j/impl/code128/EAN128AIs.properties");
			p.load(is);
			is.close();
		} catch (Exception e) {
			// TODO: handle exception
			System.err.println("Could not load org/krysalis/barcode4j/impl/code128/EAN128AIs.properties");
			e.printStackTrace();
		}
...
//		canDoChecksumADD = true;
	...
	private EAN128AI(String id, String spec, byte lenID, byte type, byte len){
		this(id, lenID, new byte[]{type}, new byte[]{len}, new byte[]{len}, new byte[]{CheckDigit.CDNone});
		fixed = true;
	}
...
//	}
	...
	private static void setAI(String aiName, EAN128AI ai) {
		...
		while (aiLastRelevantIdx >= 0 && !Character.isDigit(aiName.charAt(aiLastRelevantIdx))) aiLastRelevantIdx--;
		Object tmp;
	...
	private static EAN128AI parseSpecPrivate(String ai, String spec) {
		try {
			...
			throw new IllegalArgumentException("Can not Parse AI: \"" + ai + "\" spec: \"" + spec +"\" ");
		}
	}
